 ...
 *
 * The ZX81 display is generated by the CPU itself via an elaborate dance
 * with the ULA. The NMI generator fires every 207 T-states (one scanline)
 * to synchronize the CPU with the raster. INT is generated when the R
 * register's bit 6 transitions from 1 to 0, signaling the end of a
 * display line.
 *
 * I/O port decode (accent on address bus lines, active low):
 *   Write with A0=0 (e.g. 0xFE): Enable NMI generator, end VSYNC
 *   Write with A1=0 (e.g. 0xFD): Disable NMI generator
 *   Read  with A0=0 (e.g. 0xFE): Keyboard matrix; side effect: start
 *                                  VSYNC if NMI generator is off
 *
 * Written by
...
void ZX81::runFrame()
{
    ...
    while (z80_->getTStates() < machineInfo_.tsPerFrame && !paused_)
    {
        ...
        // during M1 cycles. INT is asserted on the transition of bit 6
        // from 1 to 0 (edge-triggered, not level-triggered).
...
uint8_t ZX81::coreIORead(uint16_t address)
{
    ...
    if ((address & 0x01) == 0)
    {
        ...
        // Side effect: reading port with A0=0 when NMI generator is off
        // starts VSYNC (the display blanking signal)
        if (!nmiGeneratorOn_)
        {
            vsyncActive_ = true;
        }
...
{
    // ZX81 ULA port decode (accent on address bus lines, active low):
    //
    //   A0=0 (e.g. port 0xFE): Enable NMI generator, end VSYNC
    //   A1=0 (e.g. port 0xFD): Disable NMI generator
    //
    // The ROM's first instruction is OUT (0xFD),A which disables NMI
    // (A1=0). Later, OUT (0xFE),A enables NMI (A0=0) to start display
    // generation.
    //
    // Note: Both conditions can be true simultaneously (e.g. port 0xFC
    // has both A0=0 and A1=0). In practice, the ROM never does this.

    // Any port write ends VSYNC
    vsyncActive_ = false;

    // A0 low → Enable NMI generator, end VSYNC, MIC output
    if ((address & 0x01) == 0)
    {
